...
(() => {
  ...
  function paintFlagsMaskAndUnknown(flagsInput) {
    ...
    const box = $(Q.flagsUnknown); if (box) {
      box.innerHTML = ''; unknown.forEach(c => {
    ...
    try {
      ...
      if (txtA) {
        ...
        if (!pickerA) {
          ...
          pickerA.title = 'Alterar cor do automap';
      ...
      if (txtL) {
        ...
        if (!pickerL) {
          ...
          pickerL.title = 'Alterar cor da luz';
...
function ensureExportUI() {
  ...
  label.textContent = 'Exportar para versão:';
  ...
  (versionManager?.versions ?? []).forEach((v, i) => {
    ...
    opt.textContent = `${v.name || v.value || 'desconhecida'}  (dat=${datHex}, spr=${sprHex})`;
  ...
  async function __runSprDatExport(opts) {
    ...
    if ('showDirectoryPicker' in window) {
      try {
        ...
        alert('✅ Exportado na pasta escolhida.');
        return;
      } catch (e) {
    ...
    else alert('Exportação cancelada ou falhou.');
  }
  ;

  window.confirmExportSprDat = async function () {
    if (!spr || !dat) { alert('Primeiro carregue os arquivos'); return; }
...
window.handleFolderSelection = async function (input) {
  ...
  if (status) status.innerHTML = 'Analisando arquivos...';
  ...
  if (!sprFile || !datFile) {
    if (status) status.innerHTML = '<span style="color:var(--danger)">❌ Arquivos .spr e .dat não encontrados na pasta.</span>';
    return;
  }
  ...
  if (status) status.innerHTML = `✅ Encontrados: ${sprFile.name}, ${datFile.name}${xmlFile ? ' e versions.xml' : ''}`;
...
window.startEditor = async function (manualVersion, manualDat, manualSpr) {
  ...
  if (!datFile || !sprFile) {
    return alert('São necessários pelo menos os arquivos .dat e .spr');
  }
  ...
  const setProgress = (p) => {
    ...
    progressTxt.textContent = `Carregando... ${p}%`;
  };
  ...
  try {
    ...
    if (versionFile) {
      try {
        ...
        console.warn("Não foi possível carregar versions.xml", e);
      }
    }
    ...
    if (!ver) {
      detectedVersion = { name: 'Autodetectada / Desconhecida', dat: datSig, spr: sprSig };
    ...
    if (vi) {
      vi.innerHTML = `
        <strong>Versão:</strong> ${detectedVersion.name}<br>
    ...
    console.error(err);
    alert('❌ Erro durante o carregamento: ' + err.message);
  }
};
